...
public sealed partial class Game
{
    ...
    /// <summary>The line-of-sight modifier (FreeCol <c>model.modifier.lineOfSightBonus</c>): the scout role grants it, and Hernando de Soto grants +1 to <b>all</b> the player's units — ships included (Col1; we widen FreeCol's <c>navalUnit=false</c> scope, <c>86d3kgbtj</c>).</summary>
    ...
    /// <c>model.modifier.lineOfSightBonus</c> +1 — folded for <b>every</b> unit the de Soto holder owns, <b>ships
    /// included</b>. The original 1994 game lifted ALL units' sight with de Soto; FreeCol's classic modifier is scoped
    /// <c>navalUnit=false</c> (land only), which we deliberately widen here for Col1 fidelity (<c>86d3kgbtj</c>). The
    ...
    private int LineOfSightOf(Unit unit)
    {
        ...
        {
            sight = ApplyGoodsModifiers(owner, LineOfSightBonusId, sight); // de Soto +1 for EVERY unit incl. ships (Col1, 86d3kgbtj); no father → unchanged
        }
        return sight;
    }
    ...
    /// boxes) falls back to humanising the key's camelCase short part via <see cref="Naming.Humanize(string)"/>, so a
