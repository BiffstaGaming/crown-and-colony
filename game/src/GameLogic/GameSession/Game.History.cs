...
/// happened, the turn it happened on, and a ready-to-read description. Presentation reads these for the History
/// report; the engine appends them as the events occur.
/// </summary>
/// <param name="Kind">The category of event (colony founded / war declared / father elected).</param>
...
public sealed record HistoryEvent(HistoryEventKind Kind, int Turn, string Description);
...
public sealed partial class Game
{
    ...
    /// <summary>Appends a notable event to the human's <see cref="History"/>, stamped with the current <see cref="Turn"/>.</summary>
    private void RecordHistory(HistoryEventKind kind, string description) =>
        _history.Add(new HistoryEvent(kind, Turn, description));
