...
/// carrier to it, and the carrier auto-hauls each turn â€” delivering what a stop doesn't want, loading what it does,
...
public class TradeRouteTests
{
    ...
    public void AssignTradeRoute_IsCarrierOnly()
    {
        ...
        Unit colonist = game.SpawnUnit(Classic.Unit("model.unit.freeColonist"), new Position(1, 0)); // space 0 â†’ not a carrier
    ...
    // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Europe stops (86d3e4bcp, GAP B) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    private const string Galleon = "model.unit.galleon";

    /// <summary>
    /// A 3Ã—1 coastal strip â€” plains port colony Alpha at (0,0) holding 200 sugar, ocean at (1,0), high seas at (2,0) â€”
    ...
    public void CreateTradeRoute_AcceptsAEuropeStop()
    {
        ...
        TradeRoute route = game.CreateTradeRoute(game.HumanPlayer, "Export run",
            [new TradeRouteStop(alpha.Id, [Sugar]), TradeRouteStop.Europe([])]); // Alpha â†’ Europe
        Assert.Equal(2, route.Stops.Count);
        Assert.True(route.Stops[1].IsEurope);
        Assert.Empty(game.ValidateTradeRoute(route)); // a Europe stop is a valid location â†’ no warnings
    }
    ...
    public void AEuropeStopRoute_HaulsColonyGoodsToEurope_AndSellsThem()
    {
        ...
        {
            game.EndTurn(); // pick up at Alpha â†’ sail to Europe â†’ sell â†’ sail home â†’ repeat
        }
    ...
    public void AEuropeStop_SurvivesSaveRoundTrip_WithoutAVersionBump()
    {
        ...
        Assert.Equal(65, SaveGame.CurrentVersion);         // Europe stop adds no save field of its own (existing TradeRouteStop shape); 58 comes from other slices
    }
    ...
    public void ARouteLessGame_OmitsTheV43Tokens_AndStaysCurrent()
    {
        ...
        Assert.DoesNotContain("NextTradeRouteId", json); // omit-when-default (counter still 1) â†’ byte-identical to v44
    ...
    public void RemoveTradeRoute_DeletesTheRoute_AndUnassignsItsCarrier()
    {
        ...
        game.RemoveTradeRoute(game.HumanPlayer, 999); // unknown route â†’ no-op, no throw
    }
    ...
    public void NextTradeRouteId_SurvivesSaveLoad_SoIdsAreNotReusedAfterDeleting()
    {
        ...
        TradeRoute c = game.CreateTradeRoute(game.HumanPlayer, "C", Stops()); // id 3 â†’ counter now 4
        ...
        Assert.Equal([1, 2, 4], loaded.HumanPlayer.TradeRoutes.Select(r => r.Id)); // A, B, D â€” all distinct, no reuse
    }
    ...
    public void APreV45Save_WithoutTheCounter_FallsBackToMaxIdPlusOne()
    {
        ...
        // No persisted counter â†’ fall back to max(1,2)+1 = 3 (the legacy behaviour, preserved for old saves).
