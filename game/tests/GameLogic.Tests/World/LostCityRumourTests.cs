...
public class LostCityRumourTests
{
    ...
    // ---- Seven Cities of Gold â€” the finite Cibola list (86d3fpxrc, FreeCol NameCache.getNextCityOfCibola) ----
    //
    ...
    // find (FreeCol ServerUnit's CIBOLA â†’ "Fall through, found all the cities" â†’ case RUINS).

    /// <summary>Resolves a CIBOLA rumour for a fresh free colonist on an empty land tile, returning the (game, outcome). 4224 â†’ Cibola; then the treasure/ruins amount roll.</summary>
    ...
    public void EachCibolaFind_DecrementsTheCount_AndSpawnsTheBigTreasure()
    {
        ...
        {
            (_, Game.LostCityRumourType outcome) = ResolveOneCibola(game, amountRoll: 0); // 0 â†’ dxÂ·300 = 2400 treasure
            Assert.Equal(Game.LostCityRumourType.Cibola, outcome);
    ...
    public void TheEighthCibola_AfterTheListIsExhausted_DegradesToAnOrdinaryRuinsFind()
    {
        ...
        // The 8th CIBOLA roll: the list is empty, so it degrades to RUINS. Amount roll 0 â†’ 0Ã—300 + 50 = 50 gold (a small
        // find â€” straight gold, no train). The outcome label stays Cibola (the roll drawn), but it resolves as ruins.
        (_, Game.LostCityRumourType outcome) = ResolveOneCibola(game, amountRoll: 0);

        Assert.Equal(Game.LostCityRumourType.Cibola, outcome);            // the rumour rolled was still Cibolaâ€¦
        Assert.Equal(goldBefore + 50, game.HumanPlayer.Gold);            // â€¦but it paid a small ruins find in gold
    ...
    {
        // Degrading to RUINS uses the ruins amount formula: a large roll still becomes a treasure train (the ruins â‰¥ 500 branch).
        Game game = Game.New(Classic, Seed);
        for (int i = 0; i < Game.CibolaCityCount; i++)
        {
        ...
        // 8th Cibola, exhausted â†’ ruins; amount roll 5 â†’ 5Ã—300 + 50 = 1550 (â‰¥ 500) â†’ a treasure train (a ruins-sized one).
    ...
    {
        Game game = Game.New(Classic, Seed);
        ResolveOneCibola(game, amountRoll: 0); // find one city â†’ 6 remain
    ...
    public void TheExhaustedCount_SurvivesReload_AndKeepsDegradingToRuins()
    {
        ...
        Assert.Equal(goldBefore + 50, reloaded.HumanPlayer.Gold); // 50 gold, no train â€” proving the persisted 0 still degrades
    }
