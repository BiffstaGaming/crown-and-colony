...
> **Playable = UI path + L3 player-flow test (the wiring-regression bar, kanban 86d3f62r5).** An L3 case must drive the **real player control** — press/emit the actual `Button`, fire the actual `PopupMenu` item id, simulate the actual key — **not** call the underlying `Game` oracle or the controller's `public` command directly. A test that skips the control (e.g. `controller.OpenEuropePanel()` instead of pressing `UI/EuropeButton`) still passes if the button is disconnected, so a wiring regression sails through CI. Assert the *player-visible* outcome (panel visible, unit moved, colony captured). When a control is genuinely **un-wired** (no seam exists, or the id is mis-bound), **report it** (spawn a follow-up) rather than asserting the current broken behaviour or adding the wiring in a test-only change — e.g. the right-click "Centre here" item was found to be mis-bound (`AddItem(label, -1)` collides with Godot 4.6's auto-assign-to-index sentinel, so its id is 0, not −1, and the `case CentreId` arm never fires).

> **A test that mutates a process-global autoload must restore it (shared-state hygiene).** The `/root/Settings` `SettingsService` is a **process singleton** shared across the whole L3 run and backed by `user://settings.cfg`. A case that changes a live setting (autosave period, UI scale, colourblind mode) and does not restore it leaks that value into every later case — and if any case `Save()`s while the value is dirty, into the on-disk `settings.cfg` for the next run too. This was a real cross-suite flake: driving the autosave-period SpinBox to 5 left the shared autoload (and then disk) at 5, so `SaveLoadTests.EndingATurn_WritesTheAutosave` — which relies on the **default period 1** so a single turn-end writes the autosave — failed on the *second* full-suite run. Any settings-mutating L3 case must restore the **default** value (not a pre-read value, which may itself be polluted) in a `finally`, and `Save()` when the leak can reach disk, so it self-heals a `settings.cfg` an earlier run may have dirtied.
...
> **The reset must also protect suites that *don't* drive input themselves (kanban 86d3fq0dj drag-state flake, 2026-07-01).** The `Input` singleton is process-global across the *whole* run, so a suite that only presses buttons/emits signals — but loads scenes that read `Input` — still inherits whatever keys/buttons the *previous* suite left flagged held. This was a real CI-only flake: `NegotiationPanelTests.TradeAtRivalColony…` (signals only, no simulated input) failed under the run order where it followed `InputTests`, because `InputTests` left keys flagged pressed and that bled a spurious action into the fresh negotiation scene. The failing suite must carry the **same** `[BeforeTest]`/`[AfterTest]` `ResetGlobalInputState` (releasing the *full* InputTests key set, not just the mouse button) so it starts clean whichever suite ran before it.

> **Every simulated drag must RELEASE and free its preview (closes kanban 86d3fd0gp).** L3 drives drag-and-drop by invoking the `Control` callbacks directly (Godot can't synthesise a real mouse-drag headlessly). The catch: `Control.SetDragPreview()` asserts `get_viewport()->gui_is_dragging()` — true only *during a genuine drag*. Calling the real `_GetDragData` override outside a drag trips that assert (the repeated `ERROR: Condition "!get_viewport()->gui_is_dragging()" is true.` in CI logs) **and orphans the preview `Control`** it was handed (an orphan-node warning per drag case). L3 drag tests must therefore call the preview-free payload seam (`EuropeDragSource.BuildDragPayload()`), **not** `_GetDragData` — production still routes through `_GetDragData` and gets its preview during a real drag. A simulated drag driven the full way (real press→move) must always emit the matching button *release* so `gui_is_dragging()` returns false at case end. This removed **22 orphaned preview Controls** (11 drag cases × 2 nodes) from the L3 run.
