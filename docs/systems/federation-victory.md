...
- `FederationPanel` (presentation) — renders the oracles and forwards the two commands; no rules. **Designed treatment (WS2.7):** on the parchment/wood theme it shows a five-step **phase tracker** (the current stage lit), each colony's support as a **styled gauge** (recessed trough, support-coloured fill — barn-red / ochre / federation-green by how it sits against the referendum bar — and a gold **threshold marker** at the bar, read from the new `Game.ReferendumSupportThreshold` oracle), a gold-edged **Convention Points** chip, and title/header in the theme's `ColonyTitle`/`SectionHeader` variations. Pure presentation (colours re-declared locally from `ColonyTheme`, the `EuropePanel` precedent).
...
| L3 Interaction | Yes (has UI) | `FederationPanelTests` — renders phase + Convention Points + six region rows; Call-Convention disabled with reason until thresholds met; **WS2.7** the designed treatment is guarded (the five-step `PhaseTracker`, each region `Bar` carrying `fill`/`background` stylebox overrides + the `ReferendumMark`, the `ColonyTitle` title variation). **`PioneerAttainedPanelTests` (WS2.7)** — the celebration renders the figure name/body/category/perk and hugs its content (a sizing regression guard); electing a figure through a live `GameController` fires the popup automatically. | ✅ |
