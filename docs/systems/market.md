...
| **Last verified** | 2026-06-20 @ market-prices report tab (`86d3dmn6d`) |
...
- **Market report tab (`86d3dmn6d`):** `ColonyReportPanel`'s **Market** tab is pure presentation (ADR-006) — it enumerates `Game.Ruleset.GoodsTypes.Where(IsTradeable)` and, per good, reads `Game.Market.BidPrice`/`AskPrice` and derives the boycott flag from `Market.Arrears(id) > 0` (equivalently `!CanTrade(id)`). No write members are touched. Sales/income columns (FreeCol `ReportTradePanel`'s `getSales`/`getIncomeBeforeTaxes`/`getIncomeAfterTaxes`) are scoped out — they need an unported per-good trade accumulator (and a save bump), so the tab shows the **price + boycott** subset only.
...
- [x] **Market report tab** (`86d3dmn6d`, FreeCol `ReportTradePanel` price subset) — `ColonyReportPanel`'s Market tab lists every tradeable good's bid/ask + boycott status, read-only over `Game.Market`. Presentation-only; no save/RNG change. See §2/§3.
- [ ] **Market report — sales/income columns:** add FreeCol's `getSales`/`getIncomeBeforeTaxes`/`getIncomeAfterTaxes` per-good accumulator (needs a save bump) to extend the report tab beyond prices.
