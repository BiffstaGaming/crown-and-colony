...
| | |
|---|---|
| **Status** | Implemented (multi-player: the human + **active** foreign powers (explore/found AI) + native nations; native AI + AI economy/combat in FP-5/6) |
| **Last verified** | 2026-06-14 @ FP-4 (minimal foreign-power AI) |
...
- The game has several players: **you** (the human), three **foreign powers** (rival European nations), and the **native nations**. The foreign powers now **play for themselves** (a simple computer player): each lands its colonists and ship on the map far away from you, founds a colony, and then sends its remaining colonist out to explore. The native nations still sit still for now (their computer player comes later).
...
- **Foreign-power AI (FP-4):** `RunForeignPowerTurn` drives each power's units in stable by-id order — a colonist founds a colony where it stands while the power has fewer than `MaxAiColonies`, else steps one tile toward the nearest tile the power has not explored (`StepTowardNearestUnexplored`); ships and non-founders idle. It is a flat priority switch, **not** a planner. All choices draw from the player's own RNG stream (`RandomFor(player)`), so it is replay-stable and never disturbs the human's stream 0. The foreign powers have **no economy yet** (no trade/immigration/liberty — FP-5).
- **Turn ring:** `EndTurn` walks `_players` from `_currentPlayerIndex` (via `NextPlayerIndex`), calling `RunPlayerTurn` on each — the human's colonies produce + it accrues liberty/immigration; a foreign power runs the AI; natives are inert — then runs the world steps (sailing, native alarm decay, movement reset, `Turn++`) once and returns to the human.
...
|---|---|---|---|
| L1 Unit | Always | `MultiPlayerTests` (foreign powers land + run the AI, `EndTurn_RunsForeignPowerAi`, `ForeignPowerAi_IsReplayStable_ForAFixedSeed`, multi-element save), `OwnerTests` (owner-id seam: enemy/fog isolation, owner round-trip), `SaveGameTests.V19Save_LoadsAsSingleHumanPlayer` (fold → one human), `LoadedGame_ContinuesIdenticalRandomSequence` (stream 0 determinism) | ✅ |
...
| L4 Visual | No new screen | golden screenshots unchanged (rivals land far + are fog-gated) | ✅ |
| L5 Soak | Always | 25 seeds × 200 turns with active rivals: invariants hold + byte-identical save round-trip; turn cost within the 2 ms budget | ✅ |
...
- [x] AI: explore/move/found (FP-4 ✅) — foreign powers land, found, and explore on their own RNG streams. [ ] AI economy (FP-5), AI combat + diplomacy/stance (FP-6); native AI; wire native units to their player id.
