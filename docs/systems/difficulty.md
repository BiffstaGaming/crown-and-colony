...
| | |
|---|---|
| **Status** | In development (slices 1–3: the parse + overlay infrastructure, the founding-father factor, units-that-use-no-bells, the four government limits, and the natives group (land-price / native-demands / rumour-difficulty) are routed through it; the rest of the tuning constants follow in later slices) |
| **Last verified** | 2026-06-18 @ natives group routed (`86d3c9y08` slice 3) |
...
- **Routed so far (slices 1–3):** `model.option.foundingFatherFactor` (24/32/40/48/56 across the levels — see [founding-fathers](founding-fathers.md)); `model.option.unitsThatUseNoBells` (2 on every classic level — bell upkeep); the **four government limits** `veryGood`/`good`/`bad`/`veryBad GovernmentLimit` (medium 100/50/6/10) → `Ruleset.Difficulty.Government`, carried on `Colony.Government` and read by `Colony.ProductionBonus` — see [sons-of-liberty](sons-of-liberty.md); and the **natives group** — `landPriceFactor` (medium 60), the raw `nativeDemands` (medium 2; the `+1` demand-dx and `(5−x)·50` relief transforms stay in code), and the raw `rumourDifficulty` (medium 2; the `10−x` reward-dx transform stays in code) — see [natives](natives.md), [lost-city-rumours](lost-city-rumours.md). **The derived transforms keep the raw spec value in the option and the formula in code.**
...
- **Not all constants are routed yet.** Slices 1–3 routed the founding-father factor, units-that-use-no-bells, the four government limits, and the natives group (land-price / native-demands / rumour-difficulty). The remaining ~8 (immigration increments — `crossesIncrement`/`lowerCapIncrease`/`priceIncrease.artillery`/`recruitPriceIncrease`, the rumour percentages `badRumour`/`goodRumour`, the `treasureTransportFee`, and the base-`gameOptions` immigration trio) are still hard-coded at their medium values and move across in later slices — each behaviour-preserving at the default, since every one already holds its medium value.
