...
| | |
|---|---|
| **Status** | In development (slices 1–4: the parse + overlay infrastructure and most tuning constants — founding-father factor, units-that-use-no-bells, government limits, the natives group, the rumour percentages, and the immigration/recruit/artillery increments — are routed through it; only the treasure-transport fee, the base-`gameOptions` immigration trio, and player-selectable+persisted level remain) |
| **Last verified** | 2026-06-18 @ percentage + immigration options routed (`86d3c9y08` slice 4) |
...
- **Not all constants are routed yet.** Slices 1–4 routed the founding-father factor, units-that-use-no-bells, the four government limits, the natives group, the rumour percentages (`badRumour`/`goodRumour`), and the immigration/recruit/artillery increments (`crossesIncrement`/`recruitPriceIncrease`/`lowerCapIncrease`/`priceIncrease.artillery`). The remaining few — the `treasureTransportFee` and the base-`gameOptions` immigration trio (`initialImmigration`/`europeanUnitImmigrationPenalty`/`playerImmigrationBonus`, which live outside the difficulty levels) — move across in slice 5, each behaviour-preserving at the default.
