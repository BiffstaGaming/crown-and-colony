...
| **Last verified** | 2026-06-21 @ Foreign Intervention Force (`86d3c9vap`); 1566 L1/L2 + 4 soak green |
...
- **Foreign Intervention Force** (the friendly counterpart, `Ruleset.InterventionForce`): a fixed composition parsed from the spec `model.option.interventionForce` (classic medium **2 colonial-regular soldiers + 2 colonial-regular dragoons + 2 artillery + 2 men-o-war**). Unlike the REF it does **not** grow via a monarch action here — it's the fixed force the foreign power lands when the rebel crosses the `interventionBells` threshold (the trigger + landing are in [independence](independence.md)). FreeCol additionally grows the standing force every `interventionTurns` (parsed but not yet applied — a follow-up).
