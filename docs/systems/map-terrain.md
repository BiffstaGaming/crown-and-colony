...
| | |
|---|---|
| **Status** | Implemented (climate-band generation + isometric FreeCol-art rendering + bonus resources with yield effects + geographic regions) |
| **Last verified** | 2026-06-19 @ P5 map regions (86d3c9w12) |
| **Code** | `game/src/GameLogic/World/` · rendering: `game/presentation/MapView.cs` |
| **Tests** | `game/tests/GameLogic.Tests/World/WorldTests.cs`, `RegionTests.cs`, `RegionGeneratorTests.cs` |
...
- `MapGenerator.Generate(ruleset, w, h, IGameRandom)`: pure function of its inputs; after building terrain it calls `RegionGenerator.Assign` and `GameMap.SetRegions` (no RNG consumed by that step).
...
|---|---|---|---|
| L1 Unit | Always | `PositionTests` (adjacency table, 8 neighbours), `GameMapTests` (row-major reads, bounds), `MapGeneratorTests` (seed determinism, border/interior shape); regions: `RegionTests` (data model + `GameMap` region layer), `RegionGeneratorTests` (polar bands, ocean quadrants + Pacific score, mountain blocks, separate-continent land regions, score formula, 75-tile split, full coverage, determinism) | ✅ |
| L2 Scenario | Always | walking-skeleton scenarios traverse generated maps with invariants; `RegionGeneratorTests.GeneratedMap_CoversEveryTile_AndIsRegionStablePerSeed` (every tile region-covered on a real map; polar-row land is arctic/antarctic) | ✅ |
...
- [ ] Rivers, lakes, multiple landmass styles (FreeCol generator features). Rivers/lakes will add RIVER/LAKE regions (currently deferred).
