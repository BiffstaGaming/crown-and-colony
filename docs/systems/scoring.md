...
| **Last verified** | 2026-06-21 @ scoring engine landed (`86d3c9vjm`) |
| **Code** | `game/src/GameLogic/GameSession/Game.cs` (`PlayerScore`, `Score`, the unit-score table + independence-bonus helper — the "Player score" section) |
...
| History events | Σ of each non-independence history event's stored score | `score += h.getScore()` (`ServerPlayer.java:881`) — **omitted today** (see deviations) |
...
- **History-event scores are omitted.** FreeCol folds region-discovery, lost-city-find and settlement/nation-destruction scores into the total via `HistoryEvent.getScore()`. Our `HistoryEvent` (in `Game.History.cs`) is a UI-facing record with **no numeric score** and is **not yet persisted**, so those summands are 0 for now. They become a follow-up once the history log carries scores.
...
**Persistence:** **nothing.** The score is *not* saved. FreeCol caches `updateScore`'s result in a serialized `score` field purely for its network protocol; we have no such need, so the value is recomputed from existing persisted state (units, colonies+liberty, congress, gold, player type, independence turn) on each read. **No save-version bump** — the design requirement (ADR-009): the default game stays byte-identical because `Score` reads existing fields and writes none.

**Purity / determinism (ADR-009):** `PlayerScore` draws **no randomness** (it constructs no generator and calls none) and **mutates no state** — verified by a test asserting `game.RandomState` and the unit/colony counts are unchanged across two reads. It is therefore safe to call from presentation any number of times.
...
- **FreeCol cross-check:** the formula and every constant are transcribed directly from `ServerPlayer.updateScore` (L858–888) and the `SCORE_*` constants (L155–171); the unit values from `specification.xml` (L1814–2213). Two documented summands (history-event scores, colony-worker unit scores) are intentionally omitted pending the supporting state — see the deviations in §2.

## 5. Open issues / TODO

- [ ] Add history-event scores once `HistoryEvent` carries a numeric score and is persisted (region discovery, lost-city finds, settlement/nation destruction penalties of −5/−50).
