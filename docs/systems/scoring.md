...
| | |
|---|---|
| **Status** | Implemented (engine read + **victory / end-of-game screen** + a **persisted high-score leaderboard** — `HighScore`/`HighScoreStore` recorded on game end, shown by `HighScoresPanel`) |
| **Last verified** | 2026-06-22 @ the `HighScore` record + `HighScoreStore` + `HighScoresPanel` high-score leaderboard (`86d3c9xb1`) |
| **Code** | `game/src/GameLogic/GameSession/Game.cs` (`PlayerScore`, **`ScoreBreakdown`**, **`RecordHighScore`**, `Score`, the unit-score table + independence-bonus helper — the "Player score" section) · `game/src/GameLogic/GameSession/ScoreComponents.cs` (itemised-breakdown value object) · **`game/src/GameLogic/GameSession/HighScore.cs`** (the leaderboard entry + `ScoreLevel` honorifics) · **`game/src/GameLogic/Persistence/HighScoreStore.cs`** (ranking/cap/dedup + JSON load/save) · history-event score: `Game.History.cs` (`HistoryEvent.Score`, `Game.HistoryEventScore`) · victory screen: `game/presentation/VictoryPanel.cs` · **high-scores screen: `game/presentation/HighScoresPanel.cs` + `game/presentation/HighScoresService.cs` (the `user://highscores.json` file I/O)** |
| **Tests** | `game/tests/GameLogic.Tests/GameSession/ScoreTests.cs` (engine, incl. `ScoreBreakdown`) · **`game/tests/GameLogic.Tests/Persistence/HighScoreStoreTests.cs` (L1 leaderboard: ranking, cap, dedup, JSON round-trip, `ScoreLevel` bands, `RecordHighScore` purity)** · `game/presentation/tests/VictoryPanelTests.cs` (L3 victory screen) · **`game/presentation/tests/HighScoresPanelTests.cs` (L3 high-scores screen)** |
...
|---|---|---|---|
| L1 Unit | Always | `game/tests/GameLogic.Tests/GameSession/ScoreTests.cs` — each summand (father +5, gold ⌊0.001×g⌋, colony liberty, unit score-value), a known-fixture total, the default new-game roster total, the independence ×2 first-place bonus, determinism, RNG-free + no-mutation, unknown/native unit → 0; **`ScoreBreakdown` summands reassemble to `PlayerScore` (incl. the +100% independence-bonus path)**. **`HighScoreStoreTests.cs` (23 cases)** — `ScoreLevel` bands, `Tidy` descending + 10-cap, `Add`/`CheckHighScore` (first score, full-board reject/replace, negative reject, same-game dedup), JSON round-trip (all fields, enum-by-name, descending-on-load, missing/corrupt → empty), and `RecordHighScore` capturing game state + being a pure read. | ✅ |
