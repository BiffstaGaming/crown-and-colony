...
| **Code** | `game/src/GameLogic/Specification/EventDef.cs` (schema + popup text), `Ruleset.cs` (`ParseHistoricalEvents`), `game/src/GameLogic/GameSession/Game.Events.cs` (runtime + effect vocabulary); **UI:** `game/presentation/EventChoicePanel.cs` (the choice modal) surfaced by `GameController.RefreshView`/`OpenEventChoicePanel` |
...
An event is: a set of **requirements** that must be true, a **weight** (how likely it is to come up when several events are eligible), and one or more **options** the player can pick between, each with **effects** (gain gold, get a free colonist, a temporary production boost, a note in your history log). Requirements come in three flavours, so an event can be **genuinely conditional** rather than just a calendar entry (design doc 01 §2):

- **State comparisons** — a year, a colony count, a rebel percentage (the same comparisons the [limits engine](events-limits.md) understands).
- **A historical figure attained** — the event only becomes available once you've recruited a particular Founding Father / Pioneer. *(The Overland Telegraph, for instance, only comes up once Charles Todd — the man who built it — is on your council.)*
- **A prior event having happened** — a **linked event**: one story beat unlocks another. *(No Merino Flock, no Wool Boom; no Gold Rush, no gold-fields immigration surge or Eureka Stockade; no land boom, no bank crash.)*
...
3. **Eligibility:** gather every `normal` event that is eligible for this player: not a spent one-shot, off cooldown, within `[earliest-year, expiry-year]`, all `<requires>` limits true, **all `<requires>` `<father>` figures attained** (in the player's Congress), **all `<requires>` `<event>` prerequisites already fired** (recorded in the game-wide fired-set), and (for the human) no event offer already pending.
...
| `<requires>` `<father>` figure not yet attained | Not eligible until that Founding Father / Pioneer is in the player's Congress (M-E) |
| `<requires>` `<event>` prerequisite has not fired | Not eligible until the linked earlier event has fired (M-E; the fired-set is game-wide) |
...
- `EventDef` — one event: id, weight, cooldown, one-shot flag, earliest/expiry year, `EventTrigger` (`Normal`/`ScenarioStart`), `Requirements` (`IReadOnlyList<Limit>`), the two **M-E prerequisite** lists `RequiredFathers` / `RequiredEvents` (`IReadOnlyList<string>`, empty = no gate), `Options`, and the **presentation-text** fields `Name`/`Prompt` (nullable — the popup title + dilemma context) with `DisplayName` falling back to the humanized id.
...
**Data sources:** the spec `<historical-events>` section (each an `<event-def>`), parsed by `Ruleset.ParseHistoricalEvents`. Deliberately a **distinct** element from the pre-existing `<events>`/`SpecEvent` (`ParseEvents`) — no element-name collision. The `<requires>` block now carries, beside its `<limit>` gates (which reuse `ParseLimit`/`ParseOperand` → `Requirements`), `<father id="…"/>` children (→ `RequiredFathers`) and `<event id="…"/>` children (→ `RequiredEvents`); effects are parsed by `ParseEventEffect`. After the full set is built, the parser **validates every `RequiredEvents` id names a defined event** (a dangling linked-event id throws `RulesetFormatException` — a typo would otherwise silently make the event unreachable; forward references are legal since the check runs post-build). Surfaced as `Ruleset.HistoricalEvents` / `Ruleset.HistoricalEvent(id)` (empty/null for classic).
...
- `IsEventEligible` — the pure eligibility read (one-shot/cooldown/year/`Requirements` via `EvaluateLimit`, plus the two M-E gates: `RequiredFathers.All(f => PlayerHasFather(player, f))` and `RequiredEvents.All(HasEventFiredById)`). `HasEventFiredById(id)` reads the game-wide `_eventLastFiredTurn` set; `PlayerHasFather(player, id)` matches the id against `player.Congress` **exactly or by dotted suffix** (so an event may name a figure by its full id or short name). Both read persisted state only — **no RNG** — so the gates are deterministic and, because the whole roll guard-returns for a ruleset with no events, they never run in classic (byte-identical). **World-event semantic:** the fired-set is game-wide while the roll is per colonial player, so in a multi-rival variant any player's firing of a prerequisite unlocks the linked event for all; the Australian Federation runs with 0 rivals, so the human is the only roller and chains fire sequentially for it.
...
|---|---|---|---|
| L1 Unit | Always | `HistoricalEventTests` — schema+parser round-trip, unknown-effect drop, duplicate-id throw; **M-E** — `Parser_ReadsRequiredFathersAndRequiredEvents`, `Parser_RejectsARequiredEventThatDoesNotExist` | ✅ |
| L2 Scenario | Always | `HistoricalEventTests` — gated draw, one-shot, cooldown, expiry, offer→choose, auto-resolve, each effect kind, scenario-start, save round-trip; `ByteStabilityTwin_EventsOff` + `ClassicDefault_FiresNoEvents_NoDrawsNoTokens`; **era bands (4c.10)** — `EraForYear_MapsBoundaryYearsToTheCorrectEra` (13 boundary/interior years), `EraFireChance_IsInertOutsideTheWindow_AndModulatedInside`, `CurrentEventEra_ReadsOffTheCalendarYear`, `EraBand_IsInertOnTheClassicCalendar_EventStillFiresEveryTurn`, `EraBand_FiresSparselyInSurvival_AndBusilyInGoldRush`, `EraBand_IsDeterministic_SameSeedSameCalendarSameCadence`; **M-E (conditional/linked)** — `RequiredFather_GatesEligibility_UntilTheFigureIsAttained`, `LinkedEvent_FiresOnlyAfterItsPrerequisiteEventHasFired`, `LinkedEvent_NeverFires_WhenItsPrerequisiteIsBlocked` (single-colonial-player games, matching Australia's 0-rival model); `AustraliaVariantTests.HistoricalEvents_WireTheMEConditionalAndLinkedPrerequisites` asserts the six shipped links on the real catalogue | ✅ |
...
- [x] **Conditional & linked events (M-E, doc 01 §2)** — events gate on gameplay prerequisites (a Historical Figure attained via `RequiredFathers`; a prior event fired via `RequiredEvents`), not only the calendar; the parser validates linked-event references. Six links wired on the Australia catalogue. Deferred within M-E (not needed yet, avoid gold-plating): *choice-specific* linking (event B unlocked only by a particular **option** of A — today any firing of A suffices) and a **state-based expiry / supersede** predicate (an event forbidden once another has fired) — `expiry-year` + `one-shot` already cover year- and once-based expiry.
...
| Date | Change | Commit |
|---|---|---|
| 2026-07-11 | **Event popup + HUD era indicator — designed treatment (WS2.7 UI polish, presentation-only)** — no rules change. The **event-choice popup** (`EventChoicePanel`) is lifted from a bare title + prompt + unlabelled buttons to the doc-19 template: the title carries the theme's `ColonyTitle` variation, the prompt reads as a centred wrapped paragraph, and the choices are **numbered** ("1." / "2." …) full-width buttons under a "Choose a response:" lead-in. The **HUD status strip** now shows the current Australian **era token** (Survival → Expansion → Colony Formation → Gold Rush → Infrastructure → Federation), read from the existing public `Game.CurrentEventEra` oracle via a presentation-side `EventEraLabel` mapper; empty for `EventEra.Pre` (every year outside the 1788–1901 window — the whole classic calendar), so the **classic HUD string is byte-for-byte unchanged**. +2 L3 (`EventChoicePanelTests`: numbered choices + lead-in + title variation; `MainSceneTests`: the era token shows for Australia (1788 → Survival), absent for classic). Render-verified (`docs/guide/img/australia-event.png`, `australia-world-hud.png`). Other WS2.7 targets (colony-maturity badges, Pioneer-attained popup) remain follow-ups. See [federation-victory](federation-victory.md) for the WS2.7 Federation-panel redesign. | (this commit) |
| 2026-07-11 | **Conditional & linked events (M-E, doc 01 §2)** — events can now gate on *play*, not only the calendar. `EventDef` gains two prerequisite lists — `RequiredFathers` (Historical Figures that must be attained, read against `Player.Congress` by exact-or-dotted-suffix match) and `RequiredEvents` (linked events that must have already fired, read against the game-wide `_eventLastFiredTurn` set) — parsed from `<requires>` `<father id>` / `<event id>` children; the parser rejects a dangling linked-event id. `IsEventEligible` ANDs both gates with the existing year/cooldown/limit gates. **No RNG, no save change** (reuses `Congress` + `EventLastFiredTurn`); classic ships no events → the gates never run → byte-identical (determinism soak green). Wired six historically-apt prerequisites on the Australia catalogue: `woolBoom`←`merinoSheep`; `payableField`/`goldImmigrationSurge`/`eurekaStockade`←`goldRush`; `bankCrash`←`marvellousMelbourne`; `overlandTelegraph`←father `charlesTodd`. (World-event semantic: the fired-set is game-wide but the roll is per colonial player — moot for Australia's 0-rival games where the human is the only roller.) +6 L1/L2 (`HistoricalEventTests` parser + gate tests using solo games; `AustraliaVariantTests` catalogue-wiring). Full L1/L2 2950 + determinism soak green. | (this commit) |
| 2026-07-09 | **Event-choice popup UI (WS1.1b)** — the 31-event narrative is now *interactive in-game*. A new `EventChoicePanel` (presentation, modelled on `EmigrationChoicePanel`) consumes `Game.PendingEventOffer` and renders the event's `DisplayName` → `Prompt` → a labelled button per option; pressing one calls `ChooseEventOption` and reports the chosen option's outcome (its `recordHistory` text). Surfaced by `GameController.RefreshView` (like the emigration prompt) — **before this, a human's multi-option dilemma (Gold Rush, Eureka, Shearers' Strike…) silently auto-resolved to the default option.** Also **authored the popup text for all 31 events** (name/prompt/labels) via a fan-out + adversarial-verify workflow, spliced into the spec (presentation attributes only — effects untouched, diff verified). The verify pass corrected one historical inaccuracy (gold-immigration "colonial population triples" → the *gold colonies'* population) and flagged sensitive events for the 4c.11 review. Single-option events keep auto-resolving (no dilemma) and log their outcome. Classic ships no events → the panel never shows. +1 L1 (`AustraliaVariantTests` authored-text) +1 L3 (`EventChoicePanelTests`: panel opens with the authored text; a choice resolves) + a render capture (`docs/guide/img/australia-event.png`). Full L1/L2 2935 + 5 soak green. **⚠ 4c.11 sign-off pending (Chris):** the Second Fleet ("Death Fleet" tragedy that grants convict labour), the First-Nations dispossession subjects (Sydney Cove / squatting / inland exploration), and the Chinese gold-rush immigration event — all authored to doc-19 tone, but sensitive; await your review. | (this commit) |
