...
| | |
|---|---|
| **Status** | In development (Phase 5: settlements placed + rendered + persisted (slice 1); tension/alarm + speak-with-chief + learn-skill (slice 3); sell-trade (slice 4); brave defenders + alarm-on-attack (slice 5b); settlement assault/plunder/destroy (slice 5c); **native AI — braves raid the human when alarmed, else wander (slice 1b)**.) |
...
| L3 Interaction | Yes (raid feedback) | `InputTests.NativeRaid_DuringEndTurn_ShowsANoticeInTheStatusBar` (an enraged brave raids on End Turn → the status bar shows a raid notice). On-map speak/learn UI still pending | ✅ |
