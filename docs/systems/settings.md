...
| | |
|---|---|
| **Status** | In development (Slice B shipped — video + audio; SFX + Music buses both live; accessibility — UI scale + colourblind palette; a Game section — autosave period; a **Key Bindings…** entry to the rebind screen) |
| **Last verified** | 2026-06-24 @ Key Bindings entry point (`86d3f0wjj`); prior @ autosave period (`86d3f0vb8`) |
| **Code** | `game/src/GameLogic/App/SettingsModel.cs` (pure), `game/presentation/SettingsService.cs` (autoload — also boot-applies key overrides), `game/presentation/SettingsScreen.cs` + `game/scenes/SettingsScreen.tscn` (now with the Key Bindings button), `game/presentation/AccessibilityPalette.cs` (colourblind colour source); key rebinding: `game/src/GameLogic/App/KeyBindingsModel.cs`, `game/presentation/KeyBindingsService.cs`, `game/presentation/KeyBindingsScreen.cs` + `game/scenes/KeyBindingsScreen.tscn` (see [hud-input.md](hud-input.md)) |
| **Tests** | `game/tests/GameLogic.Tests/App/SettingsModelTests.cs` + `KeyBindingsModelTests.cs` (L1); `game/presentation/tests/SettingsScreenTests.cs` (L3, incl. the Key Bindings button + rebind-screen cases) |
...
- **Audio** — a **Mute all** switch plus Master, Music, and Sound Effects volume sliders (0–100%). Mute silences everything at once without forgetting where the sliders are; un-mute restores them exactly.
- **Accessibility** —
...
- **Key Bindings…** — opens a screen where you can change any keyboard shortcut: click **Rebind**, press the key you'd rather use, and it takes effect immediately. **Reset** restores one shortcut; **Reset all to defaults** restores them all. Your choices are saved like every other setting (see [hud-input.md](hud-input.md) for the full key list and behaviour).
...
**What the player sees and does:** one screen with a Video, an Audio, an Accessibility and a Game section, each control applying live; a **Key Bindings…** button that opens the rebind screen; a Back button that saves and returns.
...
| Mute all | on / off | off | `AudioServer.SetBusMute("Master", on)` — silences Music + SFX at once, leaving the slider values untouched. Presentation-only state on `SettingsService` (not on `SettingsModel`), persisted as the `master_mute` key |
...
| Hidden message categories | any subset of the six `MessageCategory` values | none (all shown) | `SettingsModel.HiddenMessageCategories` — the message-log category filter; **not edited from this screen** but from the message-log panel's checkbox row (see [turns.md](turns.md)). Persisted here as a client option (FreeCol's per-type `guiShow…` message toggles) |

- **Persistence:** stored in `user://settings.cfg` (a Godot `ConfigFile`, one `[settings]` section). Written on **Back** (and immediately when a message-log category is toggled). Keys: `ui_scale` (a factor like `1.5`), `colorblind_mode` (`true`/`false`), `autosave_period` (an integer like `5`), `hidden_message_categories` (a comma-separated category-name list like `Combat,Economy`, **omitted entirely when none are hidden** so the default config is unchanged). **Note:** this is the application-settings file, *not* the game-save format — adding these options needed **no save-version bump** (the message *log itself* is persisted in the save, v59 — see [save-load.md](save-load.md)).
- **Robustness:** a missing file → all defaults; a corrupt/partial file → each bad or absent value falls back to its default, and all values are clamped into range (volumes to `[0,1]`, UI scale to `[0.75, 2.0]` (NaN → 1.0), autosave period to `[0, 100]`, unknown window mode → Windowed, anything but `true` for the colourblind flag → off, any unknown message-category token dropped). Invalid state can never be produced or persisted.
- **Persistence:** stored in `user://settings.cfg` (a Godot `ConfigFile`). The video/audio/accessibility/game options live in one `[settings]` section; **key-binding overrides** live in their own `[keybindings]` section (see [hud-input.md](hud-input.md)). Written on **Back**. Keys: `ui_scale` (a factor like `1.5`), `colorblind_mode` (`true`/`false`), `autosave_period` (an integer like `5`), and `master_mute` (`true`/`false` — the only audio key NOT backed by `SettingsModel`: it is presentation-only state on `SettingsService`, written alongside the model's keys and harmlessly ignored by `SettingsModel.FromDictionary`). **Note:** this is the application-settings file, *not* the game-save format — adding these options (and the key-binding overrides + master mute) needed **no save-version bump**. See [audio.md](audio.md) for the full audio wiring (event SFX, music, mute/volume).
...
**Key Bindings:** a `KeyBindingsButton` opens `KeyBindingsScreen` as a child overlay (itself emitting `Closed`); it is given `ProcessMode = Always` so it works over a paused game (the pause-menu path). The rebind screen and its `settings.cfg` `[keybindings]` persistence are owned by the HUD-input system — see [hud-input.md](hud-input.md) §3 (`KeyBindingsModel` / `KeyBindingsService` / `KeyBindingsScreen`). The boot-time apply of saved overrides to the `InputMap` is done by `SettingsService._Ready` (it calls `KeyBindingsService.LoadAndApply()` after applying the video/audio settings), so a rebound key works before any game scene runs.
...
| L4 Visual | Yes (has a screen) | `settings-screen` golden (`MenuGoldenTests`) — now also shows the **Key Bindings…** button; needs **CI regeneration** on the PR (the Windows dev render isn't committed; CI renders the golden on Linux per the established flow). The `KeyBindingsScreen` has no dedicated golden yet. | ⚠ CI-regenerate |
...
- [x] **Key rebinding** (`86d3f0wjj`): a *Key Bindings…* button opens the rebind screen; hotkeys are named `InputMap` actions persisted to `settings.cfg`. Owned by [hud-input.md](hud-input.md).
- [ ] Possible later additions: resolution picker, gameplay options tab, language; per-deficiency palette variants (currently one colour-universal set).
...
| 2026-06-24 | **Message-log category filter**: added `SettingsModel.HiddenMessageCategories` (`ISet<MessageCategory>`, default empty; key `hidden_message_categories`, a comma-separated name list **omitted when empty** so the default config is unchanged, unknown tokens dropped) — the message-log panel's per-category filter, persisted as a client option (FreeCol's per-type `guiShow…` message toggles). **Not surfaced in `SettingsScreen`** — edited from the message-log panel checkbox row (see [turns.md](turns.md)); the new `MessageCategory` enum lives in `GameLogic.App`. settings.cfg only — no SettingsScreen golden change. +4 L1 (`SettingsModelTests`). (`86d3f0wdv`) | _local_ |
| 2026-06-24 | **Key Bindings entry point**: added a *Key Bindings…* button to `SettingsScreen` opening `KeyBindingsScreen`, and `SettingsService._Ready` now boot-applies saved key overrides to the `InputMap`. The rebind screen + `[keybindings]` persistence are owned by [hud-input.md](hud-input.md). settings.cfg only — no save bump. `settings-screen` golden to be regenerated on CI (new button). (`86d3f0wjj`) | _local_ |
