...
**Localized text (86d3fq1w6):** `MainMenu._Ready()` ends by calling `ApplyLocalizedText()`, which sets the title, subtitle and all six buttons from `menu.*` localization keys looked up through `Loc.T(...)` (Godot `TranslationServer` wrapper) instead of the English baked into `MainMenu.tscn`. The English values are byte-identical to the old text (the `.tscn` still holds them as inert defaults), so the `main-menu` golden is unchanged; selecting French (`Loc.SetLocale("fr")`) re-renders the menu in French. This screen is the i18n **proof slice** — see [localization.md](localization.md); every other screen is still hard-coded English (the follow-up sweep).

**Data sources:** none (no ruleset/XML). Strings: `res://localization/main_menu.csv` (`menu.*` keys, `en`+`fr`) via `Loc`. Art: `res://assets/freecol/ui/map.jpg` (backdrop), `ui/bg_paper_brown.png` (parchment), `ui/colony_border.png` (frame) — all FreeCol GPL v2 (see `game/assets/freecol/PROVENANCE.md`).
...
| 2026-07-03 | **Custom-difficulty row** (`86d3fq0x7`): the `NewGameDialog` Difficulty dropdown gained a **Custom…** sixth row — selecting it opens the new per-option `DifficultyEditor` overlay (seeded from the current base level; OK validates + arms the edits, Cancel reverts the dropdown), and Start forwards the edited record via the new `NewGameDialog.PendingDifficultyOverrides` static (the callback still receives the base level). Full rules + technical detail in [difficulty](difficulty.md). **Follow-up (integrator seam):** the new-game host must read+clear the static and apply it via `Ruleset.WithDifficultyOverrides`. +5 L3 (`NewGameSetupUiTests`). | S-E (`86d3fq0x7`) |
| 2026-07-03 | **Map-import row** (`86d3fq1cg`): the `NewGameDialog` Map dropdown gained an **"Import map… (.txt / .MP)"** third row — selecting it opens a file picker, the picked file is validated on the spot (a status line under the row shows `name — W×H, N settlements`, or the parse error + a revert to Random), and Start forwards the validated import via the new `NewGameDialog.PendingImportedMap` static (the callback's `MapSource` stays Random). Full rules + technical detail in [map-terrain](map-terrain.md). **Follow-up (integrator seam):** the new-game host must read+clear the static and pass it to `Game.New(importOverride:)`. +5 L3 (`NewGameSetupUiTests`). | S-E (`86d3fq1cg`) |
| 2026-07-03 | **i18n proof slice** (`86d3fq1w6`): the title/subtitle + six buttons now render from `menu.*` localization keys via the new `Loc` wrapper over Godot's `TranslationServer` (`ApplyLocalizedText()` at the end of `_Ready()`) instead of the English baked into the `.tscn`. English is byte-identical → **`main-menu` golden unchanged**; a partial **French** locale re-renders the menu in French. This is the localization **foundation**; the full string sweep across every other screen is the documented follow-up. See [localization.md](localization.md). +6 L3 (`LocalizationTests`, incl. the MainMenu EN/FR render). | i18n (`86d3fq1w6`) |
