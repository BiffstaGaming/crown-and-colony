...
| | |
|---|---|
| **Status** | Implemented (board/sail/disembark + shared goods/passenger capacity; board/sail on the Europe screen; wagon-train goods haulage between colonies; **manual load/unload of goods between a colony warehouse and a docked carrier on the colony screen**) |
| **Last verified** | 2026-06-25 @ colony-screen cargo load/unload UI (`86d3f5y8r`) |
...
**What the player sees and does:** on the **Europe screen** ([europe.md](europe.md)) you board a recruit onto a ship and press *Sail to New World*; ships show their hold (used/free slots) and passengers. **On the map** (`86d3c9t4v`): with a land unit selected, click an adjacent (or same-tile) friendly ship to **board** it; with a ship selected, click an adjacent land tile to **disembark** a passenger ashore — the same `Board`/`Disembark` oracles the Europe screen uses (see [presentation](../modules/presentation.md)). On the **colony screen** ([colonies.md](colonies.md)), a *Cargo* section appears whenever one of your carriers is at or beside the colony, with per-carrier *Load goods…* / *Unload* controls and a hold-occupancy readout (`86d3f5y8r`).
...
**Loading and unloading goods by hand — the colony screen.** Trade routes and the Europe market move goods by sea automatically, but for a **one-off trip** you load a carrier yourself. When one of your **carriers** (a ship *or* a wagon train) is standing **on or next to** one of your colonies, the colony screen grows a **Cargo** section listing each such carrier with its **hold occupancy** ("hold 2/4"). Pick a good from the carrier's *Load goods…* list to put a hold-lot (up to 100) of it aboard from the warehouse, or press *Unload* on a goods stack already in the hold to return a lot to the warehouse. The warehouse total and the hold count update after each move. If you try to overfill the hold (no slot free) or load more than the warehouse holds, nothing happens and the status bar tells you why — the move is refused, never lost mid-action.

**Worked example (manual cargo):**
> Your inland colony has 250 ore and you want it at the coast for export. Build a wagon train there; on the colony screen it appears in the *Cargo* section showing "hold 0/2". Load ore twice — the holds fill to "2/2" (200 ore aboard, 50 left in the warehouse) and a third *Load* is refused ("The carrier has no room for that cargo."). Drive the wagon to the port colony, open its screen, and *Unload* the ore into that warehouse.
...
- **Manual colony↔carrier cargo on the colony screen** (`86d3f5y8r`): the colony screen now exposes the `LoadFromColony`/`UnloadToColony` oracles to the player for a one-off haul (previously only the trade-route auto-hauler and the Europe market moved goods by sea). Goods move a **hold-lot at a time** (≤100, clamped to warehouse stock on load / to the carried amount on unload), matching the Europe screen's ×100 buy/sell lot; the player repeats the action for a bigger stack. The "own colony only" ownership gap (a carrier may load/unload at *any* adjacent colony, ours or not) is unchanged — still a documented gap shared with ships, not introduced or closed here.
...
**Colony-screen cargo UI** (`86d3f5y8r`, presentation-only — ADR-006): `ColonyPanel.CargoArea` renders a *Cargo* section per `DockedCarriers()` (the player's `IsCarrier` units on or adjacent to the colony) only when that list is non-empty — so a carrier-free colony's screen (and its visual golden) is unchanged. Each carrier shows `CargoSlotsUsed/CargoCapacity`, a *Load goods…* `OptionButton` over the warehouse stock, and one *Unload* button per goods stack aboard (`Unit.Cargo`). The controls forward to thin `GameController.LoadColonyCargo`/`UnloadColonyCargo` commands that wrap `LoadFromColony`/`UnloadToColony` in a `try/catch (InvalidMoveException)`: on success they set a status notice, on a guard failure they show the exception's reason in the status bar — the move is never thrown to the UI. The lot is `min(100, available)`. After a command (which already refreshes the HUD + set the notice) the panel re-renders itself via a deferred `RebuildDeferred` (not the usual `Changed`, which would re-run the host refresh and clear the just-set notice before the player sees it).
...
| L3 Interaction | Yes (Europe + colony screens) | `EuropePanelTests.BoardThenSail_SendsAColonistHome` (board a colonist + sail via the real screen buttons) — see [europe.md](europe.md); `ColonyPanelTests.CargoSection_LoadsAndUnloadsGoods_AndSafelyRefusesAFullHoldOrEmptyWarehouse` (load a lot via the *Load goods…* picker, unload via the *Unload* button, then a full-hold and an out-of-stock load both refused with no throw to the UI) | ✅ |
| L4 Visual | Colony golden regenerates | the fresh-colony golden (`colony-panel-seed424242`) **gains the *Cargo* section** — at seed 424242 the starting **caravel sits at (3,11), adjacent to the founded colony at (4,12)**, so the panel now shows it docked. **Deliberate, visible golden change** — the PNG is **regenerated on Linux CI**, not committed from local (Windows render differs); matches the settings-screen golden-regen pattern. | 🔄 regen on CI |
...
- [x] **Wagon-train goods haulage** (`86d3c9t3g`): a wagon train (any `IsCarrier` land unit) `LoadFromColony`/`UnloadToColony` to move goods colony-to-colony overland. Follow-ups: an "own colony only" ownership gate (shared with ships); ~~map-side load/unload UI~~ (done — `86d3f5y8r`); trade routes (`86d3c9rq1`).
- [x] **Manual colony↔carrier cargo UI** (`86d3f5y8r`): the colony screen's *Cargo* section loads/unloads goods between the warehouse and a docked/adjacent carrier (ship or wagon), a hold-lot at a time, with hold occupancy shown and the engine guards surfaced in the status bar.
- [ ] **Own-colony-only ownership gate** for `LoadFromColony`/`UnloadToColony` (still loads/unloads at any adjacent colony, ours or not — shared with ships).
- [ ] Capacity-aware **auto-loading**; multi-ship cargo transfer; an amount **slider/spinbox** (today it is a fixed ≤100 lot, repeated for more).

## Changelog

| Date | Change | Commit |
|---|---|---|
| 2026-06-25 | **Manual colony↔carrier cargo UI** (`86d3f5y8r`): the colony screen grows a *Cargo* section listing each docked/adjacent player carrier (ship or wagon) with its hold occupancy, a *Load goods…* picker (warehouse → hold, a ≤100 lot clamped to stock) and per-stack *Unload* buttons (hold → warehouse). Routed through thin `GameController.LoadColonyCargo`/`UnloadColonyCargo` commands that wrap the existing `LoadFromColony`/`UnloadToColony` oracles in a try/catch and surface the `InvalidMoveException` reason in the status bar (no throw to the UI). Section hidden when no carrier is present (golden unchanged). Presentation-only (ADR-006); no logic/save/RNG change. +1 L3 (`ColonyPanelTests.CargoSection_LoadsAndUnloadsGoods_AndSafelyRefusesAFullHoldOrEmptyWarehouse`); 2194 L1/L2 + 15 ColonyPanel L3 green. | P5 (`86d3f5y8r`) |
