...
| **Last verified** | 2026-06-22 @ colony sight radius 1→2, data-driven from `visible-radius` (Wave 10) |
...
- [ ] **Coronado `exposedTilesRadius`** (follow-up): `model.event.seeAllColonies` should reveal each colony at its line-of-sight **+** the father's `model.modifier.exposedTilesRadius` (classic additive +3 → an 11×11 block). That father modifier isn't modelled yet, so Coronado reveals only each colony's `ColonySightRadius` for now.
