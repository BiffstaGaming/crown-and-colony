...
| **Last verified** | 2026-06-21 @ pioneer-improvement spec parse + applicability (road / plow / clear-forest now parsed with scopes, required-role, tool cost, tile-type-changes — not yet buildable) (`86d3dqr62`) |
...
- **River only is *placed/wired*; pioneer improvements are *parsed* but not yet buildable.** The river is the only improvement the generator places and the only one wired into yield/movement so far. The pioneer-built road / plowed-field / clear-forest types are now **fully parsed** — applicability scopes, `required-role`, `expended-amount` (tool cost), `add-work-turns`, and clear-forest's `tile-type-change` (forest → cleared base + lumber) — so the rules are in the model (`TileImprovementType.AppliesTo`, `ChangeFrom`), but the **build action** (a pioneer spending turns to lay them, consuming tools) is the next slice. The fish-bonus types are parsed-through (their +fish is applied directly by `Game.TileYieldPotential`, not via a placed improvement).
...
|---|---|---|---|
| L1 Unit | Always | `World/TileImprovementTests.cs`: river model attributes vs. spec, all eight goods deltas, movement rule (both-endpoints / never-free / never-dearer edges); **`PioneerImprovementTypeTests`** — road/plow/clear-forest parsed attributes vs. spec (required-role, tool cost, work turns, expose%), applicability scopes (road=any land not water; plow=cleared farmland not forest/elevation/arctic/water; clear-forest=forest only), clear-forest tile-type-changes (each forest → cleared base + lumber 20, scrub 10), river parses natural, pioneer role `CanImproveTerrain`. `World/WorldTests.cs` → `RiverTests` (river parse + placement + yield/movement folding + v47 round-trip); `Colonies/TileWorkerTests.cs` → river-mouth fish | ✅ |
...
- [x] **Other improvement types — spec parse + applicability**: road / plowed / clear-forest now parsed in full (scopes, required-role, tool cost, work turns, tile-type-changes) with `TileImprovementType.AppliesTo`/`ChangeFrom`; `RoleType.CanImproveTerrain` identifies the pioneer role.
- [ ] **Pioneer build action**: a tooled pioneer spends turns to lay road/plow/clear-forest, consuming tools (the build action + per-turn work + save bump + the road/plow/clear effects). The parsed model above is the foundation for this.
