...
| | |
|---|---|
| **Status** | Implemented (full game state: multi-player `Players[]`, per-player RNG streams, diplomacy stances/tensions, native settlements + interaction + missions + convert progress, ship repair, per-colony liberty, unit standing orders, colony build queues, Lost City Rumours, native land claims, map regions, treasure trains, custom-house export settings, escalated Europe unit prices, per-colonist worker types + on-the-job experience + school training; save v35) |
| **Last verified** | 2026-06-19 @ map regions (save v35: per-tile region layer + region table) |
...
- Saves carry a format `Version` (currently **35** — the authoritative source is `SaveGame.CurrentVersion`); older saves still load with sensible defaults for fields that didn't exist yet (see the changelog), and v1 saves default units to free colonists and reveal fog around units.
