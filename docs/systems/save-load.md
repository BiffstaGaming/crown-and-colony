...
| **Last verified** | 2026-06-19 @ goto destinations (save v36: per-unit goto destination) |
...
- Saves carry a format `Version` (currently **36** — the authoritative source is `SaveGame.CurrentVersion`); older saves still load with sensible defaults for fields that didn't exist yet (see the changelog), and v1 saves default units to free colonists and reveal fog around units.
