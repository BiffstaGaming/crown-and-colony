...
| | |
|---|---|
| **Status** | Implemented (full game state: multi-player `Players[]`, per-player RNG streams, diplomacy stances/tensions, native settlements + interaction + missions + convert progress, ship repair, per-colony liberty, unit standing orders, colony build queues, Lost City Rumours, native land claims, treasure trains, custom-house export settings, escalated Europe unit prices, per-colonist worker types + on-the-job experience + school training; save v34) |
| **Last verified** | 2026-06-18 @ missionaries (save v34: resident missionary + convert progress) |
