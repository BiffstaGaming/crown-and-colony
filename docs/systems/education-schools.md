...
- **Deviations / not yet modelled:**
  ...
  - Building **upkeep** (gold) is unmodelled project-wide. There is no *assign-a-teacher* UI yet (any in-window expert in a school is a teacher); a future placement command must apply the same min/max-skill gate. Student selection is automatic (classic `allowStudentSelection=false`).
