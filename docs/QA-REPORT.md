...
> **Snapshot** taken 2026-06-15, latest on `main` (*foreign-powers wave FP-1→FP-6b (stance machine) — multi-player + active foreign-power AI with an economy + **diplomacy stance/tension that follows tension (war→cease-fire→peace)**; ADR-019*).
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 396 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 396 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **396** | ✅ | every push |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (396)
