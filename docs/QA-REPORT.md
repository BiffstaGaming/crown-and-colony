...
> **Snapshot** taken 2026-06-16, latest on `main` (*native AI (1b); rival rendering + CheckMove colony guard (1c-1); foreign-power retaliation (1c-2); Pocahontas + Magellan founding-father effects; naval combat (1c-3a); ship damage + repair (1c-3b, save v21); naval cargo loot (1c-3c); native interaction UI; privateer piracy (1c-3d-i); Francis Drake (1c-3d-ii); colonial-colony capture (1c-3e); ambient native alarm; foreign-AI colony capture (1c-3f); native colony pillage (goods **+ gold-steal**); colony fortification defence bonus (stockade/fort/fortress +100/+150/+200%); La Salle (free stockade at pop 3); colony capture plunder (sack the treasury); AI besiege pathing; **human defeat** — the human is wiped out (no colonies + no units) and the HUD reports it; **game-over flow** — a defeat overlay + a disabled End Turn stop the game (presentation-only); + **native tribute demands** — a hostile brave beside a colony it can't storm demands goods/gold the human pays or refuses in a pay/refuse modal; + **colony tile-work picker** — assign a colonist to a surrounding tile for a chosen non-food good (lumber/ore/cotton/…); + **colony join/leave/arm** — move colonists in and out of a colony and arm them; + **FreeCol-look colony screen** — the colony view rebuilt after FreeCol's, on its **brown parchment skin**: a production goods bar, a left column (colonist-portrait population strip + isometric tile view with colonists on worked tiles + construction), a right buildings grid of real building images with staffing, an outside-units row, and a warehouse goods-icon bar (banded layout, opaque background — verified at 1920×1040; Sons-of-Liberty bar deferred, no per-colony liberty data); ADR-019; **+ title/settings front-end (2026-06-18, `a804549`)** — a main menu (boot scene), settings screen + persistence, in-game pause menu, reusable info popup, a five-slot save/load dialog, and a bundled Cardo UI font with the first UI visual goldens*). *(Note: the L1+L2 count below predates both this arc and the other workstream's GameLogic growth — the [CI runs](https://github.com/BiffstaGaming/crown-and-colony/actions) are the live source.)*
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 582 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 582 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **582** | ✅ | every push |
...
| **L3 Interaction** | Real scenes driven by simulated input/signals (incl. 1 scene E2E + the Europe screen + click-to-attack + native-raid notice + foreign-colony-capture notice + human-defeat banner + game-over overlay + unit rendering + owner colour + ship-under-repair + native-settlement panel + native-tribute-demand modal + colony tile-work picker + join/leave colony + arm colonist + click-to-move tile workers + Sons-of-Liberty bar + FreeCol-art colony screen: parchment skin + isometric tiles + buildings image grid + Sons-of-Liberty bar + effective tile yields) | GdUnit4 | 35 | ✅ | every push (CI) |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (582)
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category=Soak"    # L5 (4)
dotnet test game/CrownAndColony.csproj --settings game/gdunit.runsettings                 # L3+L4 (40), needs GODOT_BIN
```
...
### Front-end screens — `main-menu`, `settings-screen`, `pause-menu`, `info-popup`
What they verify: the title/settings front-end (2026-06-18) renders as designed — parchment/wood framing, the bundled Cardo serif, button layout/labels. The first UI goldens, enabled by the licence-clear font; a looser 2% tolerance absorbs cross-platform text antialiasing. Definitions: [menu-goldens.md](visual-tests/menu-goldens.md).
...
- **The front-end screens now have visual goldens** — main menu, settings, pause menu, info popup (see [menu-goldens.md](visual-tests/menu-goldens.md)), unblocked by bundling a licence-clear UI font (Cardo, SIL OFL). The **colony** and **Europe** screens still have none (covered by L3 interaction tests only — the map L4 captures still hide the UI layer). Tracked: [task 86d3b4653](https://app.clickup.com/t/86d3b4653).
