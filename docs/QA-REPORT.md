...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 523 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 523 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **523** | ✅ | every push |
...
| **L3 Interaction** | Real scenes driven by simulated input/signals (incl. 1 scene E2E + the Europe screen + click-to-attack + native-raid notice + foreign-colony-capture notice + human-defeat banner + game-over overlay + unit rendering + owner colour + ship-under-repair + native-settlement panel) | GdUnit4 | 26 | ✅ | every push (CI) |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (523)
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category=Soak"    # L5 (4)
dotnet test game/CrownAndColony.csproj --settings game/gdunit.runsettings                 # L3+L4 (31), needs GODOT_BIN
```
