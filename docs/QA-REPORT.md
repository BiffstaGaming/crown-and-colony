...
> **Snapshot** taken 2026-06-14, latest on `main` (*Phase 5 slice 5b — attack action: ownership, roles/equipment, brave defenders, combat outcomes + native alarm*).
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 330 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 330 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **330** | ✅ | every push |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (330)
