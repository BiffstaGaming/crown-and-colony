...
> **Snapshot** taken 2026-06-14, latest on `main` (*foreign-powers wave FP-1→FP-6a — multi-player + active foreign-power AI with an economy + **recorded diplomacy stance/tension**; ADR-019*).
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 381 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 381 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **381** | ✅ | every push |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (381)
