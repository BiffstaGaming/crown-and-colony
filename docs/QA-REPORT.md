...
> **Snapshot** taken 2026-06-15, latest on `main` (*native AI (1b); rival/own-unit rendering + CheckMove colony guard (1c-1); foreign-power retaliation combat (1c-2); + **Pocahontas founding-father effect** — on election all native alarm resets to Happy and future combat alarm gains are halved; ADR-019*).
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 431 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 431 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **431** | ✅ | every push |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (431)
