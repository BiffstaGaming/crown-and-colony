...
> **Snapshot** taken 2026-06-15, latest on `main` (*native AI (1b); rival rendering + CheckMove colony guard (1c-1); foreign-power retaliation (1c-2); Pocahontas + Magellan founding-father effects; + **naval combat (1c-3a, sink-only)** — ship-vs-ship evade/sink (cargo + passengers down), foreign warships attack at war; ADR-019*).
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 448 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 448 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **448** | ✅ | every push |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (448)
