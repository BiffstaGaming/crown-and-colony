...
> **Snapshot** taken 2026-06-15, latest on `main` (*native AI (1b); rival rendering + CheckMove colony guard (1c-1); foreign-power retaliation (1c-2); Pocahontas + Magellan founding-father effects; naval combat (1c-3a); + **ship damage + repair (1c-3b, save v21)** — a beaten ship is damaged and limps to Europe to repair over 5 turns (cannot sail/load/board meanwhile), sinking only on a great loss / no repair location; ADR-019*).
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 460 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 460 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **460** | ✅ | every push |
...
| **L3 Interaction** | Real scenes driven by simulated input/signals (incl. 1 scene E2E + the Europe screen + click-to-attack + native-raid notice + unit rendering + owner colour + ship-under-repair) | GdUnit4 | 22 | ✅ | every push (CI) |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (460)
