...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 489 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 489 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **489** | ✅ | every push |
...
| **L5 Soak** | 25-seed × 200-turn runs (active foreign economies + native AI, native ambient alarm) + provoked native-raid & foreign-war invariants + per-turn perf budget | xUnit | 4 | ✅ | nightly |
| | | | **521** | **all green** | |

Reproduce locally (toolchain in [CLAUDE.md](../CLAUDE.md)):
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (489)
