...
> **Auto-generated from CI** on 2026-07-25 at commit `8f859fb` (save format **v75**). Do not hand-edit — this file is rewritten by [`scripts/generate-qa-report.py`](../scripts/generate-qa-report.py) on every CI run from the actual `dotnet test` results.
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 2997 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 2997 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **2997** | ✅ | every push |
...
| **L3 Interaction** | Real scenes driven by simulated input/signals | GdUnit4 | 424 | ✅ | every push (CI) |
...
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (2997)
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category=Soak"    # L5 (6)
dotnet test game/CrownAndColony.csproj --settings game/gdunit.runsettings                 # L3+L4 (442), needs GODOT_BIN
```
