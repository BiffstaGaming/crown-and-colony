...
> **Auto-generated from CI** on 2026-08-15 at commit `1dd7bfd` (save format **v76**). Do not hand-edit — this file is rewritten by [`scripts/generate-qa-report.py`](../scripts/generate-qa-report.py) on every CI run from the actual `dotnet test` results.
...
|---|---|---|---:|:--:|---|
| **L1 Unit** | Rules, formulas, state transitions (engine-free) | xUnit | included in 3049 | ✅ | every push |
| **L2 Scenario** | Scripted multi-turn games, FreeCol cross-checks | xUnit | included in 3049 | ✅ | every push |
| **L1+L2 total** | (the engine-free `GameLogic` suite) | xUnit | **3049** | ✅ | every push |
...
| **L3 Interaction** | Real scenes driven by simulated input/signals | GdUnit4 | 434 | ✅ | every push (CI) |
| **L4 Visual** | Golden-screenshot diff of the rendered map/UI | GdUnit4 + custom diff | 18 | ✅ | every push (CI) |
| **L5 Soak** | Multi-seed long runs + per-turn perf budget | xUnit | 6 | ✅ | nightly |
| | | | **3507** | **all green** | |

Reproduce locally (toolchain in [CLAUDE.md](../CLAUDE.md)):
```
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category!=Soak"   # L1+L2 (3049)
dotnet test game/tests/GameLogic.Tests/GameLogic.Tests.csproj --filter "Category=Soak"    # L5 (6)
dotnet test game/CrownAndColony.csproj --settings game/gdunit.runsettings                 # L3+L4 (452), needs GODOT_BIN
```
...
### `about-panel`
![about-panel golden](../game/tests/visual/goldens/about-panel.png)
...
### `difficulty-editor`
![difficulty-editor golden](../game/tests/visual/goldens/difficulty-editor.png)

### `europe-panel`
![europe-panel golden](../game/tests/visual/goldens/europe-panel.png)

### `help-panel`
![help-panel golden](../game/tests/visual/goldens/help-panel.png)
...
### `new-game-dialog`
![new-game-dialog golden](../game/tests/visual/goldens/new-game-dialog.png)
