...
**Created:** 2026-07-15. **Status:** scoping complete; awaiting the first slice.
...
1. **Slice 1 — Copper (pattern-setter).** A new distinct mined **export** good: `<goods-type model.goods.copper>` + `<market>`; a `model.resource.copper` on hill/mountain terrain that already has a base ore/mineral output, plus a small base `copper` production on that terrain (≥1); optional native `SeedGoods`. Icon deferred (name-only) or a GPL placeholder. Proves the whole "add a distinct good" pattern end-to-end with no retargeting. Faithful: the SA copper boom (Burra/Kapunda), a major raw export.
