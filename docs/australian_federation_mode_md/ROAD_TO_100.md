...
- **1.1 Event-choice popup UI.** A styled dialog that consumes `PendingEventOffer` and calls `ChooseEventOption`, to doc 19's event template (historical context → game context → 2–4 choice buttons). Wire into `GameController.RefreshView` (same pattern as `EmigrationChoicePanel`). Sombre styling + no reward-popup for sensitive events (doc 13). **This turns the whole 31-event system on.** L3 + L1 wiring tests.
...
- **4.4 Full multi-clause Pioneer perks.** Complete the deferred perk clauses from the M-C remap (e.g. Macarthur's Merino Stud + free sheep; Mort's spoilage removal; Todd's distance-decay halving; the movement/drought/stock-route clauses) as their backing systems land. L1/L2.
