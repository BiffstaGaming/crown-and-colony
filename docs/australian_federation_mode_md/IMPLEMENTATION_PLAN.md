...
> **Progress — 2026-07-09:** **Phases 0–2 SHIPPED and Phase-4 substantially landed.** "Australian Federation" is fully playable and Australian-reskinned end-to-end (all CI-green on `main`):
> - **Skeleton + map + Pioneers** (`86d3kwtf9`/`86d3kwtp5`/`86d3kwtjb`) — the real 60×40 Australia continent (de-staggered, NZ removed), the 25 Australian Pioneers under the Federation Convention.
> - **Full reskin** — goods (Wool/Gold/Civic Voice), units (Convict/Emancipist/Free Settler/Digger/Shepherd), buildings (Government Stores/Wool Shed/Rum Still/Tobacco Shed/…), UI chrome (Federationists/Imperial Loyalists/Imperial Pressure/First Nations), and the Colopedia/Help instructional prose — all display-only via a variant-scoped `Naming`/`GameVariant.DisplayOverrides` seam; classic byte-identical.
> - **Content** — 1788–1901 timeline; Australian place-names (Sydney-first) incl. a nation-less-human default bucket; six named colony regions + colony-start scenarios; First Nations communities **named** (Eora/Kulin/Noongar/Yolngu/Arrernte/Larrakia/Yawuru/Wangkatja — naming pass only); a variant 1788 opening cinematic; the **1788–1901 event catalogue** (batches 1–3, ~30 events) with **era frequency-bands**; and **novel content** (new economy/civic buildings, tile improvements, Hargraves' gold rush, bespoke Pioneer effects).
> - **Federation victory loop (4a, ADR-021)** — per-region Federation Support (from Civic Voice) → Convention Points → phase machine → seeded referendum → **Commonwealth proclamation victory**, with a Federation HUD panel (verified in-game). Save **v72**; classic byte-identical (Federation off for classic). Imperial Pressure kept political-only; the 5 victory grades deferred. **On documented ADR-021 defaults — pending Chris's sign-off.**
>
> **Remaining for 100%:** the deep **First Nations redesign (4b, ADR-022)** — sensitive, sign-off-gated (only the naming pass is done); **art production** (asset shortlist researched — needs Chris's approval + First Nations cultural consultation); Federation **grades/clauses/quotas** + AI-pursues-Federation (deferred refinements); Pioneer **hard date-gates** (4c.9); **content balance** (4d.8 — needs playtest). The sensitive framings already merged (intro beats, tribe names, Eureka/Shearers events) await Chris's review.
