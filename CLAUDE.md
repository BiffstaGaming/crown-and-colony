...
- **Post-work summary (every prompt / area of work)**: after finishing each prompt or discrete area of work, **prepend a new dated entry to `PostWorkSummary.md`** (repo root) using the entry format defined at the top of that file — a concise, at-a-glance record (requested / did / status / changed / decisions / **scheduled-next** / follow-ups / needs-you). Always name the **specific work item scheduled next** (plus its kanban task id). This is Chris's quick read of "what just happened and what's next"; it complements but does **not** replace the durable cross-session record in the ClickUp Session Log (doc 06). Keep entries short; newest first. Commit it with the work it describes. **Also paste this exact entry into your final chat reply to Chris** — writing it only to the file does not count as "provided"; Chris reads it in the conversation, so the templated entry must appear in the response every single time, in addition to being committed.
