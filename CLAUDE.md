...
- **Post-work summary (every prompt / area of work)**: after finishing each prompt or discrete area of work, **prepend a new dated entry to `PostWorkSummary.md`** (repo root) using the entry format defined at the top of that file — a concise, at-a-glance record (requested / did / status / changed / decisions / next / needs-you). This is Chris's quick read of "what just happened"; it complements but does **not** replace the durable cross-session record in the ClickUp Session Log (doc 06). Keep entries short; newest first. Commit it with the work it describes.
